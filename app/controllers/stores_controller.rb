class StoresController < ApplicationController
	...
	def index
		@user = current_user
# if there is a dist_search param then show only stores within X distance of search
		if params[:search]
			@stores = Store.within(2, :origin => params[:search])
# if there is a store_name search & no geoloc then show stores with name
		elsif params[:search_by_name] && cookies[:lat_lng].nil?
			@stores = Store.where('name iLIKE ?', "%#{params[:search_by_name]}%").order("name DESC")
# if there is a searchby store name & geoloc then show stores ordered by distance
		elsif params[:search_by_name] 
			lat_lng = cookies[:lat_lng].split("|")
			@stores = Store.where('name iLIKE ?', "%#{params[:search_by_name]}%").by_distance(origin: lat_lng)
